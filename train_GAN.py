import numpy as np
from face.model_GAN import Generator,Discriminator
...
def model_fn(features,labels,mode):
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        ...
        return tf.estimator.EstimatorSpec(mode=mode, train_op=train_op,loss=loss1,
...
def _train_input_fn():
    return input_fn(record_file="D:/pycharm/dataset/record/data_train_point_and_line2.record",batch_size=4,num_epoches=10,shuffle=True)
def _predict_input_fn():
    return input_fn(record_file="D:/pycharm/dataset/record/data_train_point_and_line2.record",batch_size=2,num_epoches=1,shuffle=False)
